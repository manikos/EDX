...
def buildItems():
    return [Item(n,v,w) for n,v,w in (('a', 175, 10),
                                      ('b', 90, 9),
                                      ('c', 20, 4))]
##                                      ('vase', 50, 2),
##                                      ('book', 10, 1),
...
def chooseBest2(items, maxWeight):
    if len(items) == 0 or maxWeight <=0:
        return (0, ())
    first = items[0]
    w = first.getWeight()
    rest = items[1:]
    print '1 alt', str(first), w, [str(i) for i in rest]

    v1, t1 = chooseBest2(rest, maxWeight)
    print 'v1=', v1

    print '2 alt', str(first), w, [str(i) for i in rest]
    v2, t2 = chooseBest2(rest, maxWeight -w)
...
def testBest():
    items = buildItems()
    pset = powerSet(items)
    val, taken = chooseBest2(items, 20)
    print 'Total value of items taken =',val
    for item in taken:
        print '  ', item
